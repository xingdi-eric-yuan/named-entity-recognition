...
void
trainNetwork(const std::vector<std::vector<std::string> > &x, const Mat &y, std::vector<Cvl> &CLayers, std::vector<Fcl> &HiddenLayers, Smr &smr, const std::vector<std::vector<std::string> > &tx, const Mat &ty, std::unordered_map<string, Mat> &wordvec){

    if (is_gradient_checking){
        std::vector<Mat> tpx;
        Mat tpy = Mat::zeros(1, 1, CV_64FC1);
        getSample(x, &tpx, y, &tpy, 1, wordvec);
    ...
    cout<<"****************************************************************************"<<endl
        ...
        // define the velocity std::vectors.
        ...
        std::vector<Mat> v_hl_b;
        std::vector<Mat> hlWd2;
        std::vector<Mat> hlbd2;
        ...
        std::vector<std::vector<Mat> > v_cvl_W;
        std::vector<std::vector<double> > v_cvl_b;
        std::vector<std::vector<Mat> > cvlWd2;
        std::vector<std::vector<double> > cvlbd2;
        for(int cl = 0; cl < CLayers.size(); cl++){
            std::vector<Mat> tmpvecW;
            std::vector<double> tmpvecb;
            std::vector<Mat> tmpvecWd2;
            std::vector<double> tmpvecbd2;
        ...
        for(int epo = 1; epo <= training_epochs; epo++){
            for(; k <= iter_per_epo * epo; k++){
                ...
                std::vector<Mat> batchX;
                Mat batchY = Mat::zeros(y.rows, batch_size, CV_64FC1); 
                getSample(x, &batchX, y, &batchY, batch_size, wordvec);    
                ...
                batchY.release();
                $$LOG saveConvLayer(CLayers, path); $$_LOG
            } 
            if(!is_gradient_checking){
                cout<<"Test training data: "<<endl;;
                testNetwork(x, y, CLayers, HiddenLayers, smr, wordvec);
                cout<<"Test testing data: "<<endl;;
                testNetwork(tx, ty, CLayers, HiddenLayers, smr, wordvec);
            }
        }
        ...
        v_hl_b.clear();
        std::vector<Mat>().swap(v_hl_b);
        v_cvl_W.clear();
        std::vector<std::vector<Mat> >().swap(v_cvl_W);
        v_cvl_b.clear();
        std::vector<std::vector<double> >().swap(v_cvl_b);
        hlWd2.clear();
        std::vector<Mat>().swap(hlWd2);
        cvlWd2.clear();
        std::vector<std::vector<Mat> >().swap(cvlWd2);
    }
}