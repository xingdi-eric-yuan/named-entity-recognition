...
get_layers_config(string &str){
    ...
    for(int i = 0; i < layers.size(); i++){
        ...
        switch(type){
            case 0:{
                int kw = get_word_int(layers[i], "KERNEL_WIDTH");
                int kh = get_word_int(layers[i], "KERNEL_HEIGHT");
                int ka = get_word_int(layers[i], "KERNEL_AMOUNT");
                double wd = get_word_double(layers[i], "WEIGHT_DECAY");
                int phori = get_word_int(layers[i], "POOLING_HORI");
                int pvert = get_word_int(layers[i], "POOLING_VERT");
                bool is3ch = get_word_bool(layers[i], "IS_3CH_KERNEL");
                bool lrn = get_word_bool(layers[i], "USE_LRN");
                convConfig.push_back(ConvLayerConfig(kw, kh, ka, wd, phori, pvert, lrn));
                break;
            }case 1:{
...
readConfigFile(string filepath, bool showinfo){
    ...
    word_vec_len = get_word_int(str, "WORD_VEC_LENGTH");
    ...
    for(int i = 0; i < convConfig.size(); i++){
        ...
        cout<<"KernelWidth = "<<convConfig[i].KernelWidth<<endl;
        cout<<"KernelHeight = "<<convConfig[i].KernelHeight<<endl;
        cout<<"KernelAmount = "<<convConfig[i].KernelAmount<<endl;
        cout<<"WeightDecay = "<<convConfig[i].WeightDecay<<endl;
        cout<<"PoolingHori = "<<convConfig[i].PoolingHori<<endl;
        cout<<"PoolingVert = "<<convConfig[i].PoolingVert<<endl;
    ...
    cout<<"word vec length = "<<word_vec_len<<endl;
