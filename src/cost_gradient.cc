...
void
getNetworkCost(std::vector<Mat> &x, Mat &y, std::vector<Cvl> &CLayers, std::vector<Fcl> &hLayers, Smr &smr){

    int nsamples = x.size();
    // Conv & Pooling
    unordered_map<string, Mat> cpmap;
    unordered_map<string, std::vector<Point> > locmap;
    convAndPooling(x, CLayers, cpmap, locmap);
    std::vector<Mat> P;
    std::vector<string> vecstr = getLayerKey(nsamples, CLayers.size() - 1, KEY_POOL);
    for(int i = 0; i<vecstr.size(); i++){
        P.push_back(cpmap.at(vecstr[i]));
    }
    convedWidth = P[0].cols;
    Mat convolvedX = concatenateMat(P, nsamples);
    P.clear();

    // full connected layers
    std::vector<Mat> nonlin;
    std::vector<Mat> hidden;
    std::vector<Mat> acti;
    std::vector<double> factor;
    hidden.push_back(convolvedX);
    acti.push_back(convolvedX);
    std::vector<Mat> bernoulli;
...
//        tmpacti = sigmoid(tmpacti);
    ...
    // bp - full connected
    std::vector<Mat> delta(hidden.size());
    std::vector<Mat> deltad2(hidden.size());
...
//    deltad2[deltad2.size() - 1] = deltad2[deltad2.size() -1].mul(pow(dsigmoid_a(acti[acti.size() -1]), 2.0));
    ...
    for(int cl = CLayers.size() - 1; cl >= 0; cl --){
        ...
        std::vector<string> deltaKey = getLayerKey(nsamples, cl, KEY_DELTA);
        for(int k = 0; k < deltaKey.size(); k ++){
            ...
            Mat upDelta = UnPooling(cpmap.at(deltaStr), pVert, pHori, pooling_method, locmap.at(locstr));
            Mat upHessian = UnPooling(cpmap.at(hessianStr), pVert, pHori, pooling_method, locmap.at(locstr));
        ...
        if(cl > 0){
            for(int k = 0; k < convConfig[cl - 1].KernelAmount; k ++){
                std::vector<string> prevUD = getSpecKeys(nsamples, cl, cl - 1, k, KEY_UP_DELTA);
                std::vector<string> prevUH = getSpecKeys(nsamples, cl, cl - 1, k, KEY_UP_HESSIAN);
        ...
        for(int j = 0; j < convConfig[cl].KernelAmount; j ++){
            Mat tpgradW = Mat::zeros(convConfig[cl].KernelHeight, convConfig[cl].KernelWidth, CV_64FC1);
            double tpgradb = 0.0;
            Mat tpWd2 = Mat::zeros(tpgradW.size(), CV_64FC1);
            double tpbd2 = 0.0;
            std::vector<string> convKey = getKeys(nsamples, cl, j, KEY_UP_DELTA);
            ...
            std::vector<string> convKey2 = getKeys(nsamples, cl, j, KEY_UP_HESSIAN);
    ...
    locmap.clear();
    unordered_map<string, std::vector<Point> >().swap(locmap);
    acti.clear();
    std::vector<Mat>().swap(acti);
    delta.clear();
    std::vector<Mat>().swap(delta);
    bernoulli.clear();
    std::vector<Mat>().swap(bernoulli);
    nonlin.clear();
    std::vector<Mat>().swap(nonlin);
}
